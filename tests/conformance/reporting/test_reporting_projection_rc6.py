...
async def test_warm_production_proof_cannot_hide_a_changed_protocol_pin(
    ...
    async with production_harness("postgres", tmp_path / "pin.sqlite", adcp_version=RC6) as h:
        ...
        async with mounted.client() as client:
            ...
            if mutation == "value":
                monkeypatch.setattr(h.production.handler, "_adcp_version", RC3)
            else:
                monkeypatch.setattr(h.production.handler, "get_adcp_version", lambda: RC3)
            for transport in ("mcp", "a2a-0.3", "a2a-1.0"):
...
@pytest.mark.parametrize("backend", ["memory", "postgres"])
async def test_rc6_nearest_captured_generation_and_historical_scope_filters(backend):
    async with feed_harness(backend) as h:
...
async def test_rc6_period_start_forecast_retains_civil_dst_and_offset_instants(
            ...
            if reconciled:
                ...
                _, accepted = await mounted.call(
                    ...
                    "sync_reporting_receipts",
                    {
                        ...
                        "idempotency_key": "rc6-production-receipt",
