...
def _load_schema_registry(state: _LoaderState) -> None:
    """Register every modular schema by its bundle path and canonical ``$id``.
    ...
    for file in _walk_json(state.root.root):
        ...
        try:
            ...
            continue
        # New modular documents may omit $id while their callers still use
        # canonical bundle URLs. Resolve those documents locally too: fetching
        # a different remote representation can break their fragment refs.
