...
#   filename:  _brand_discovery.json
#   timestamp: 2026-09-05T19:08:37+00:00
...
class LocalizedName(RootModel[dict[str, str]]):
    root: Annotated[dict[str, str], Field(max_length=1, min_length=1)]


class LocalizedScalar1(RootModel[list[LocalizedName]]):
    root: Annotated[
        list[LocalizedName],
        Field(
            description="A legacy plain string or an ordered set of one-locale string maps. Producers MUST NOT repeat a locale after replacing '_' with '-' and applying the AdCP locale-tag canonicalization profile. Readers resolve the requested locale using the document default_language fallback contract and MUST NOT choose an arbitrary available translation.",
            min_length=1,
        ),
    ]


class LocalizedScalar(RootModel[str | LocalizedScalar1]):
    root: Annotated[
        str | LocalizedScalar1,
        Field(
            description="A legacy plain string or an ordered set of one-locale string maps. Producers MUST NOT repeat a locale after replacing '_' with '-' and applying the AdCP locale-tag canonicalization profile. Readers resolve the requested locale using the document default_language fallback contract and MUST NOT choose an arbitrary available translation."
        ),
    ]
...
class LocalizedStringListEntry(RootModel[dict[str, list[str]]]):
    root: Annotated[dict[str, list[str]], Field(max_length=1, min_length=1)]


class LocalizedStringList1(RootModel[list[LocalizedStringListEntry]]):
    root: Annotated[
        list[LocalizedStringListEntry],
        Field(
            description="A legacy plain string list or an ordered set of one-locale maps whose values are complete string lists. Producers MUST NOT repeat a locale after replacing '_' with '-' and applying the AdCP locale-tag canonicalization profile.",
            min_length=1,
        ),
    ]


class LocalizedStringList(RootModel[list[str] | LocalizedStringList1]):
    root: Annotated[
        list[str] | LocalizedStringList1,
        Field(
            description="A legacy plain string list or an ordered set of one-locale maps whose values are complete string lists. Producers MUST NOT repeat a locale after replacing '_' with '-' and applying the AdCP locale-tag canonicalization profile."
        ),
    ]
...
class Colors(AdCPBaseModel):
    __pydantic_extra__: Dict[str, ColorValue]
    model_config = ConfigDict(
        extra='allow',
    )
...
class Fonts(AdCPBaseModel):
    __pydantic_extra__: Dict[str, FontRole]
    model_config = ConfigDict(
        extra='allow',
    )
...
class Tone(AdCPBaseModel):
    voice: Annotated[
        ...
        Field(
            description='High-level voice descriptor, either a legacy plain string or localized values.'
        ),
    ] = None
    attributes: Annotated[
        LocalizedStringList | None,
        Field(
            description='Personality traits that characterize the brand voice. Localized lists are selected as a whole.'
        ),
    ] = None
    dos: Annotated[
        LocalizedStringList | None,
        Field(
            description='Guidance for copy generation - what TO do. Localized lists are selected as a whole.'
        ),
    ] = None
    donts: Annotated[
        LocalizedStringList | None,
        Field(
            description='Guardrails to avoid brand violations - what NOT to do. Localized lists are selected as a whole.'
        ),
    ] = None
...
class BrandIdModel(RootModel[str]):
    root: Annotated[
        str,
        Field(
...
class Separator(AdCPBaseModel):
    ...
    type: Annotated[Type, Field(description='Separator style.')]
...
class TypeScale(AdCPBaseModel):
    __pydantic_extra__: Dict[str, TypeScaleEntry]
    model_config = ConfigDict(
        extra='allow',
    )
...
class DefaultLanguage(AdCPBaseModel):
    pass
...
class Property(AdCPBaseModel):
    ...
    type: Annotated[PropertyType, Field(description='Property type')]
...
class Collection(AdCPBaseModel):
    ...
    role: Annotated[
        TalentRole | None, Field(description="This person's role on the collection")
    ] = None
...
class RightsAgent(AdCPBaseModel):
    ...
    right_types: Annotated[
        list[RightType] | None, Field(description='Types of rights available', min_length=1)
    ] = None
    countries: Annotated[
        list[Country1] | None,
...
class BrandAgentEntry(AdCPBaseModel):
    ...
    type: Annotated[BrandAgentType, Field(description='Functional role of this agent')]
    url: Annotated[
        AnyUrl,
        Field(
            description="Agent endpoint URL (MCP or A2A). Callers comparing a brand's declared agent URL against another value (e.g., resolving 'is this the agent that signed this artifact?' or matching against a discovery cache) MUST canonicalize both sides per the AdCP URL canonicalization rules, not byte-equality. After the publication chain and an authorized request-signing key verify, this exact entry's canonical URL is the stable Agent identity input for principal mapping; the domain, JWKS URI, kid, and key material are credential or trust facts beneath it, not the Agent identity. See docs/reference/url-canonicalization."
        ),
    ]
    id: Annotated[
        str,
...
class BrandKitOverride(AdCPBaseModel):
    ...
    logo: Annotated[ImageAsset | None, Field(description='Override logo asset.')] = None
    colors: Annotated[Colors1 | None, Field(description='Override brand colors (hex strings).')] = (
        None
    )
    voice: Annotated[
        str | None,
...
class IdentityRelyingParty(AdCPBaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
    issuer: Annotated[
    ...
    scheme: Annotated[
        str | None, Field(description='Proof scheme and version, e.g. "world_id_v4".')
    ] = None
...
class BrandDiscovery(
    ...
    root: Annotated[
        ...
        Field(
            ...
            examples=[
                {
                    '$schema': 'https://adcontextprotocol.org/schemas/3.2.0-rc.1/brand.json',
