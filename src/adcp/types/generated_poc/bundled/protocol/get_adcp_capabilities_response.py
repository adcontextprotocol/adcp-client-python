...
class ChangeFeed3(AdCPBaseModel):
    model_config = ConfigDict(
        extra='allow',
    )
...
class PrimaryKey(RootModel[str]):
...
class ReportingProfile(AdCPBaseModel):
    ...
    primary_keys: Annotated[list[PrimaryKey], Field(min_length=1)]
...
class Schedule(AdCPBaseModel):
    ...
    alignment: Annotated[
        Alignment,
        Field(
            description='Calendar used to establish exact period boundaries. source_timezone is an explicit upstream reporting clock, such as an ad server network timezone, that may differ from the AdCP account timezone. The obligation echoes resolved timestamps and source timezone.',
            title='Reporting Schedule Alignment',
        ),
    ]
...
class SupportedFinalityEnum(StrEnum):
    snapshot = 'snapshot'
    official = 'official'
...
class ProducerIdentity(AdCPBaseModel):
    ...
    cloud: Annotated[Cloud | None, Field(title='Reporting Cloud')] = None
    region: Annotated[str | None, Field(max_length=128, min_length=1)] = None


class ReaderCompatibilityItem(RootModel[str]):
    root: Annotated[
...
class GeoPostalAreasAdditionalPropertyEnum(StrEnum):
...
class SupportedControlMode(StrEnum):
    max_impressions = 'max_impressions'
    suppress = 'suppress'
    max_impressions_and_suppress = 'max_impressions_and_suppress'


class AllowedValue(MajorVersion):
    pass


class MaxImpressionsConstraints(AdCPBaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
...
class AllowedInterval(MajorVersion):
    pass
...
class Method(AdCPBaseModel):
    ...
    orchestration: Annotated[
        Orchestration,
        Field(
            description='Party responsible for starting, configuring, and monitoring the delivery operation. Independent of destination ownership and the service that moves the data.',
    ...
    reader_compatibility: list[ReaderCompatibilityItem] | None = None
...
class Offering(AdCPBaseModel):
    ...
    reconciliation_mode: Annotated[
        ...
        Field(
            ...
            title='Reporting Reconciliation Mode',
        ),
    ]
    method: Annotated[
        Method | None,
...
class ReliabilityStatistic(AdCPBaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
    offering_id: Annotated[
...
class ReportingDelivery(AdCPBaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
    supported: Literal[True]
    ...
    reliability_statistics: Annotated[
        list[ReliabilityStatistic] | None,
        Field(
...
class Format5(AdCPBaseModel):
    ...
    format_kind: Annotated[
        FormatKind,
        Field(
            description='Discriminator naming one of the 15 AdCP 3.2 canonical creative formats, plus `custom` for adopter-defined shapes. AdCP 3.2 adds `audio_vast` for VAST-tag-delivered audio (the standards-conformant profile starts at VAST 4.1, with older versions available only as seller-declared legacy interoperability) and promotes the legacy `multi_state_display` and `multi_placement_takeover` custom shapes to `seller_rendered_stateful_display` and `coordinated_placements`. When `format_kind` is `custom`, the declaration also carries `format_shape` and `format_schema`. The enum mirrors the branches in `product-format-declaration.json` and is closed for producer validation: sellers do not mint ad-hoc values. SDKs may retain unknown future values in tolerant in-memory parsers, but that does not change versioned JSON Schema validation. A seller responding to a 3.1-or-earlier request MUST project away 3.2-only kinds and emit a compatible legacy declaration; a strict older schema correctly rejects an unprojected newer kind. See adcp#3666 and the promotion migration contract in canonical-formats.mdx.',
...
class FrequencyCapConstraints(AdCPBaseModel):
    ...
    supported_control_modes: Annotated[
        list[SupportedControlMode] | None,
        Field(
    ...
    max_impressions_constraints: Annotated[
        MaxImpressionsConstraints | None,
        Field(
            description='Exact supported max_impressions presets or range.',
            title='Frequency Cap Impression Constraints',
        ),
    ] = None
    window_constraints: Annotated[
        list[FrequencyCapIntervalConstraints] | None,
        Field(
...
class AggregateFrequencyCapping(FrequencyCapConstraints):
    supported_control_modes: Annotated[
        list[Literal['max_impressions']],
        Field(
            description='FrequencyCap shapes accepted by the product. max_impressions_and_suppress means both controls may appear together and are enforced with AND semantics.',
            min_length=1,
        ),
...
class MediaBuy(AdCPBaseModel):
    ...
    aggregate_frequency_capping: Annotated[
        AggregateFrequencyCapping | None,
        Field(
            ...
            title='Media Buy Frequency Cap Capability',
        ),
    ] = None
