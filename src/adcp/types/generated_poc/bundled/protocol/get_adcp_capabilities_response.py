...
class Signals(AdCPBaseModel):
    ...
    discovery_modes: Annotated[
        ...
        Field(
            description="Discovery modes this signals agent supports on get_signals. 'brief' (default — every signals agent supports this): semantic discovery driven by signal_spec or signal_ids. 'wholesale': raw wholesale signals feed enumeration — caller omits signal_spec/signal_ids and the agent returns its full priced signals feed, paginated, scoped by filters/account/destinations/countries. Agents that do not declare 'wholesale' MAY return INVALID_REQUEST for wholesale calls. Absent declaration is treated as ['brief'].",
            min_length=1,
        ),
    ] = [DiscoveryMode.brief]
    features: Annotated[
...
class Transport(AdCPBaseModel):
    ...
    type: Annotated[Type11, Field(description='Protocol transport type')]
...
class Endpoint(AdCPBaseModel):
    ...
    preferred: Annotated[
        Type11 | None, Field(description='Preferred transport when host supports multiple')
    ] = None
