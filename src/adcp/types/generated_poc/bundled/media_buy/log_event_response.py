...
#   filename:  bundled/media_buy/log_event_response.json
...
class LogEventResponse3(AdCPBaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
...
class LogEventResponse4(AdCPBaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    errors: Annotated[list[Error], Field(description="Operation-level errors", min_length=1)]
    context: Annotated[
        dict[str, Any] | None,
        Field(
            description="Opaque correlation data that is echoed unchanged in responses. Used for internal tracking, UI session IDs, trace IDs, and other caller-specific identifiers that don't affect protocol behavior. Context data is never parsed by AdCP agents - it's simply preserved and returned.",
...
LogEventResponse = LogEventResponse3 | LogEventResponse4
