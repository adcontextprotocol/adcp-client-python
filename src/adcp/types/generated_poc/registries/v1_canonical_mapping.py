...
class Transform(StrEnum):
    identity = 'identity'
    singleton_array = 'singleton_array'


class ParameterMapping(AdCPBaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
    source_field: Annotated[
        str, Field(description='Field read from the matched v1 format_id object.', min_length=1)
    ]
    target_parameter: Annotated[
        str, Field(description='Field written into the projected v2 params object.', min_length=1)
    ]
    transform: Annotated[
        Transform | None,
        Field(
            description='Value transform applied during forwarding. identity copies the value; singleton_array wraps it in a one-element array.'
        ),
    ] = Transform.identity
