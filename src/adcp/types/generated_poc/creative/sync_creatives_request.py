...
from ..core import creative_assets, creative_localization, creative_revision_id
...
class Creative63(AdCPBaseModel):
...
class Creative61(AdCPBaseModel):
    model_config = ConfigDict(
        extra='allow',
    )
...
class Creative62(AdCPBaseModel):
    model_config = ConfigDict(
        extra='allow',
    )
    creative_id: Annotated[
...
class Creative64(Creative61, Creative63):
    pass


class Creative65(Creative62, Creative63):
    pass


class Creative(RootModel[Creative64 | Creative65]):
    root: Creative64 | Creative65
    def __getattr__(self, name: str) -> Any:
        """Proxy attribute access to the wrapped type."""
        if name.startswith('_'):
            raise AttributeError(name)
