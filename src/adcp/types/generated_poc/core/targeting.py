...
class AudienceInclude(RootModel[list[str]]):
    root: Annotated[
        list[str],
        Field(
...
class AxeExcludeSegment(RootModel[str]):
    root: Annotated[
        str,
        Field(
...
class AxeIncludeSegment(RootModel[str]):
    root: Annotated[
        str,
        Field(
...
class Browser(RootModel[list[browser_family.BrowserFamily]]):
    root: Annotated[
        list[browser_family.BrowserFamily],
        Field(
...
class BrowserExclude(RootModel[list[browser_family.BrowserFamily]]):
    root: Annotated[
        list[browser_family.BrowserFamily],
        Field(
...
class CollectionListExclude(RootModel[collection_list_ref.CollectionListReference]):
    root: Annotated[
...
class Demographics(RootModel[demographic_targeting_intent.DemographicTargetingIntent]):
    root: Annotated[
...
class DevicePlatform(RootModel[list[device_platform_1.DevicePlatform]]):
    root: Annotated[
        list[device_platform_1.DevicePlatform],
        Field(
...
class DevicePlatformExclude(RootModel[list[device_platform_1.DevicePlatform]]):
    root: Annotated[
        list[device_platform_1.DevicePlatform],
        Field(
...
class DeviceType(RootModel[list[device_type_1.DeviceType]]):
    root: Annotated[
        list[device_type_1.DeviceType],
        Field(
...
class DeviceTypeExclude(RootModel[list[device_type_1.DeviceType]]):
    root: Annotated[
        list[device_type_1.DeviceType],
        Field(
...
class GeoCountries(RootModel[list[GeoCountry]]):
    root: Annotated[
        list[GeoCountry],
        Field(
...
class GeoCountriesExclude(RootModel[list[GeoCountriesExcludeItem]]):
    root: Annotated[
...
class GeoMetrosExclude(RootModel[list[GeoMetrosExcludeItem]]):
    root: Annotated[
        list[GeoMetrosExcludeItem],
        Field(
...
class GeoRegions(RootModel[list[GeoRegion]]):
    root: Annotated[
        list[GeoRegion],
        Field(
...
class GeoRegionsExclude(RootModel[list[GeoRegionsExcludeItem]]):
    root: Annotated[
...
class PropertyListExclude(RootModel[property_list_ref.PropertyListReference]):
    root: Annotated[
        property_list_ref.PropertyListReference,
        Field(
...
class StoreCatchments(RootModel[list[StoreCatchment]]):
    root: Annotated[
        list[StoreCatchment],
        Field(
...
class GeoProximityItem2(AdCPBaseModel):
    model_config = ConfigDict(
        extra='allow',
    )
    lat: Annotated[
...
class GeoProximity(RootModel[list[GeoProximityItem2]]):
    root: Annotated[
        list[GeoProximityItem2],
        Field(
            description='Target users within travel time, distance, or a custom boundary around arbitrary geographic points. Multiple entries use OR semantics — a user within range of any listed point is eligible. For campaigns targeting 10+ locations, consider using store_catchments with a location catalog instead. Seller must declare support in get_adcp_capabilities.',
            min_length=1,
        ),
...
class DaypartTargets(RootModel[list[daypart_target.DaypartTarget]]):
    root: Annotated[
        list[daypart_target.DaypartTarget],
        Field(
            description='Restrict delivery to specific time windows. Each entry specifies days of week, an hour range, and an optional timezone that defaults to inventory_local. A concrete IANA zone uses one shared civil-time clock, while inventory_local evaluates each inventory unit in its seller-assigned local timezone. Entries are independent and MAY use different clocks.',
            min_length=1,
        ),
...
class GeoPlaces(RootModel[list[geo_place_area.GeographicPlaceArea]]):
    root: Annotated[
        list[geo_place_area.GeographicPlaceArea],
        Field(
            description='Restrict delivery to catalog-backed named places. Values MUST be stable identifiers in the declared system, not display names. Sellers must declare supported systems, countries, and place types in get_adcp_capabilities and reject unsupported entries rather than silently dropping them.',
            min_length=1,
        ),
...
class GeoPlacesExclude(RootModel[list[geo_place_area.GeographicPlaceArea]]):
    root: Annotated[
        list[geo_place_area.GeographicPlaceArea],
        Field(
            description='Exclude catalog-backed named places. Uses the same identifier-based shape as geo_places. Sellers MUST reject overlap with geo_places for the same country, system, place_type, and value.',
...
class GeoPostalAreas(RootModel[list[postal_area.PostalArea]]):
    root: Annotated[
        list[postal_area.PostalArea],
        Field(
            description='Restrict delivery to specific postal areas. Prefer the native country + postal system form. The deprecated legacy country-fused postal-system tokens remain accepted for compatibility. Seller must declare supported systems in get_adcp_capabilities.',
            min_length=1,
        ),
...
class GeoPostalAreasExclude(RootModel[list[postal_area.PostalArea]]):
    root: Annotated[
        list[postal_area.PostalArea],
        Field(
            description='Exclude specific postal areas from delivery. Prefer the native country + postal system form. The deprecated legacy country-fused postal-system tokens remain accepted for compatibility. Seller must declare supported systems in get_adcp_capabilities.',
...
class PlacementSelection(RootModel[placement_selection_1.PlacementSelection]):
    root: Annotated[
        placement_selection_1.PlacementSelection,
        Field(
            description='Purchased placement selection within the product. This constrains package inventory; it is distinct from creative_assignments[].placement_refs, which only route individual creatives within the purchased set. On create, mode selected supplies the complete selected set and mode default uses the product default. In request-side Targeting Input, a non-null value replaces this dimension, omission preserves or inherits it, and null clears it when the product permits that broader inventory set.'
        ),
    ]
