...
class Signal(AdCPBaseModel):
    ...
    signal_id: Annotated[
        ...
        Field(
            description='Universal signal identifier referencing the data provider signal definition.'
        ),
    ]
    ...
    signal_type: signal_catalog_type.SignalCatalogType
    data_provider: Annotated[str, Field(min_length=1)]
    coverage_percentage: Annotated[float, Field(ge=0.0, le=100.0)]
    deployments: Annotated[Sequence[deployment.Deployment], Field(min_length=1)]
    pricing_options: Annotated[list[vendor_pricing_option.VendorPricingOption], Field(min_length=1)]


class Payload2(AdCPBaseModel):
    model_config = ConfigDict(
        extra='allow',
    )
    product_id: str
...
class Payload4(AdCPBaseModel):
    model_config = ConfigDict(
        extra='allow',
    )
    signal_agent_segment_id: str
    signal_id: signal_id_1.SignalId | None = None
