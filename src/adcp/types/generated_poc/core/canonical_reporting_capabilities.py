# generated by datamodel-codegen:
#   filename:  core/canonical_reporting_capabilities.json
#   timestamp: 2026-08-28T20:03:29+00:00

from __future__ import annotations

from adcp.types._str_enum import StrEnum
from typing import Annotated

from adcp.types.base import AdCPBaseModel
from pydantic import ConfigDict, Field

from ..enums import available_metric, reporting_frequency
from . import (
    brand_key,
    demographic_reporting_capability,
    geo_breakdown_support,
    measurement_window,
    spot_reporting_capability,
    vendor_metric_id,
)


class DateRangeSupport(StrEnum):
    date_range = 'date_range'
    lifetime_only = 'lifetime_only'


class VendorMetric(AdCPBaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
    vendor: brand_key.BrandKey
    metric_id: vendor_metric_id.VendorMetricId


class CanonicalReportingCapabilities(AdCPBaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
    available_reporting_frequencies: Annotated[
        list[reporting_frequency.ReportingFrequency], Field(min_length=1)
    ]
    expected_delay_minutes: Annotated[int, Field(ge=0)]
    timezone: Annotated[
        str,
        Field(
            description='Explicit reporting-period timezone for this product. It may equal Account.timezone or differ when the upstream platform reports on a separate boundary.'
        ),
    ]
    supports_webhooks: bool
    available_metrics: list[available_metric.AvailableMetric]
    vendor_metrics: list[VendorMetric] | None = None
    supports_creative_breakdown: bool | None = None
    supports_format_breakdown: bool | None = None
    supports_keyword_breakdown: bool | None = None
    supports_geo_breakdown: geo_breakdown_support.GeographicBreakdownSupport | None = None
    supports_device_type_breakdown: bool | None = None
    supports_device_platform_breakdown: bool | None = None
    supports_audience_breakdown: bool | None = None
    supports_demographic_breakdown: (
        demographic_reporting_capability.DemographicReportingCapability | None
    ) = None
    supports_placement_breakdown: bool | None = None
    supports_property_breakdown: bool | None = None
    supports_collection_breakdown: bool | None = None
    supports_installment_breakdown: bool | None = None
    supports_collection_property_breakdown: bool | None = None
    supports_installment_property_breakdown: bool | None = None
    supports_placement_property_breakdown: bool | None = None
    supports_spot_breakdown: spot_reporting_capability.SpotReportingCapability | None = None
    date_range_support: DateRangeSupport
    windowed_pull_granularities: list[reporting_frequency.ReportingFrequency] | None = None
    measurement_windows: Annotated[
        list[measurement_window.MeasurementWindow] | None, Field(min_length=1)
    ] = None
