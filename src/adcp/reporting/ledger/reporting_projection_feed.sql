-- B2.4 snapshots use a new table; legacy B2.3 rows keep their original representation.
DO $migration$
BEGIN
    PERFORM pg_advisory_xact_lock(hashtext('adcp.reporting.projection.feed.schema'));
    CREATE TABLE IF NOT EXISTS reporting_projection_feed_snapshots (
        ...
        ownership_mode TEXT COLLATE "C" NOT NULL CHECK (ownership_mode IN ('absent','bindings')),
    ...
    CREATE OR REPLACE FUNCTION reporting_projection_feed_immutable()
    RETURNS TRIGGER LANGUAGE plpgsql AS $function$
    BEGIN
        RAISE EXCEPTION 'reporting feed snapshot is immutable' USING ERRCODE = '23514';
    END
    $function$;
    DROP TRIGGER IF EXISTS reporting_projection_feed_immutable ON reporting_projection_feed_snapshots;
    CREATE TRIGGER reporting_projection_feed_immutable BEFORE UPDATE OR DELETE ON reporting_projection_feed_snapshots
        FOR EACH ROW EXECUTE FUNCTION reporting_projection_feed_immutable();
END
$migration$;
