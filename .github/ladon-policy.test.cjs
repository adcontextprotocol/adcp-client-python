...
test("read-only policy CI runs exact-head parsing and upstream approval/race regressions without secrets", () => {
  ...
  assert.equal(
    ci.jobs.policy.env.LADON_ACTIONS_PATH,
    "${{ github.workspace }}/.ladon-reviewed-actions",
  );
  assert.equal(
    ci.jobs.policy.steps.find((step) => step.run?.startsWith("node --test"))[
      "working-directory"
    ],
    "consumer",
  );
  assert.equal(
    ci.jobs.policy.steps.find((step) =>
...
test("legacy approval-based retirement cannot run after the new arbiter", () => {
  assert.equal(steps.at(-1).name, "Run Ladon");
  assert.doesNotMatch(
    read(".github/workflows/ai-review.yml"),
