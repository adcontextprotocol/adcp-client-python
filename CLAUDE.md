...
1. Fix the generation script (`scripts/generate_types.py`)
2. Create a post-generation hook script
3. Add custom logic in wrapper classes, not generated classes

**Files we incorrectly modified (need proper solution):**
- `product.py` - Added model_validators for PublisherProperty and Product
- `preview_render.py` - Fixed self-referential RootModel types
- `promoted_offerings.py` - Fixed BrandManifest import reference
- `create_media_buy_request.py` - Fixed BrandManifest import reference
- `get_products_request.py` - Fixed BrandManifest import reference

**Proper solutions needed:**
1. Post-generation script to inject validators into specific classes
2. Fix datamodel-code-generator output for BrandManifest forward references
3. Fix datamodel-code-generator output for self-referential RootModels
