...
NESTED_TYPES_TO_ANALYZE = [
    ...
    ("Packages", "media_buy.update_media_buy_request"),
    ("Packages1", "media_buy.update_media_buy_request"),
]
...
def analyze_model(model_class) -> list[dict]:
    ...
    for field_name, field_info in model_class.model_fields.items():
        ...
        # Check for list[Enum]
        is_enum_list, enum_type = is_list_of(annotation, lambda t: isinstance(t, type) and issubclass(t, Enum))
        if is_enum_list:
            coercions.append({
                "field": field_name,
                "type": "enum_list",
...
def generate_code() -> str:
    ...
    lines.append(')')
    lines.append('from adcp.types.generated_poc.media_buy.get_products_request import GetProductsRequest')
    ...
    lines.append('from adcp.types.generated_poc.media_buy.update_media_buy_request import (')
    ...
    for type_name in type_order:
        ...
        for c in coercions:
            if c["type"] == "enum":
                ...
                field_comments.append(f'{c["field"]}: list[{c["target_class"].__name__} | str] | None')
            elif c["type"] == "context":
                ...
                field_comments.append(f'{c["field"]}: list[{c["target_class"].__name__}] (accepts subclass instances)')
    ...
    # Handle Packages and Packages1 together if they have same coercions
