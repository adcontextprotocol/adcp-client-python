#!/usr/bin/env python3
"""Sync AdCP schemas, skills, and selected vectors from the protocol bundle.
...
Usage:
    ...
    python scripts/sync_schemas.py --no-skills  # schemas + selected vectors
"""
...
COMPATIBILITY_VECTORS_DIR = (
    REPO_ROOT / "tests" / "conformance" / "vectors" / "products-only-brief-compatibility"
)
...
def sync_compatibility_vectors_from_bundle(bundle_root: Path, dest: Path) -> int:
    """Vendor the signed products-only compatibility vectors used by SDK tests.

    The release tarball is already checksum- and Sigstore-verified before this
    function runs. Keeping the extraction in this script preserves provenance
    and prevents a later manual download from silently drifting from the schema
    pin.
    """

    src = bundle_root / "compliance" / "test-vectors" / "products-only-brief-compatibility"
    if not src.is_dir():
        raise RuntimeError("Bundle missing products-only-brief compatibility test vectors")
    if dest.exists():
        shutil.rmtree(dest)
...
def sync_request_signing_vectors_from_bundle(
    bundle_root: Path,
    dest: Path,
    manifest_path: Path,
    *,
    ...
    """Vendor and pin request-signing vectors from the verified release bundle."""

    relative_source = Path("compliance") / "test-vectors" / "request-signing"
    src = bundle_root / relative_source
    if not src.is_dir():
        raise RuntimeError("Bundle missing request-signing compliance test vectors")
    if dest.exists():
        shutil.rmtree(dest)
    shutil.copytree(src, dest)
    ...
    manifest = (
        ...
        if manifest_path.is_file()
            ...
            "$comment": (
                "Completeness pin for the vendored AdCP request-signing conformance "
                "vectors. Generated by scripts/sync_schemas.py; do not hand-edit."
            )
        }
    )
    manifest["spec_version"] = effective_version
    manifest["source"] = (
        f"adcontextprotocol/adcp @ v{effective_version} -- "
        f"dist/compliance/{effective_version}/test-vectors/request-signing/"
    )
...
def _sync_one(
    ...
    *,
    ...
    with tmpdir:
        ...
        if sync_compatibility_vectors:
            try:
                vector_count = sync_compatibility_vectors_from_bundle(
                    bundle_root, COMPATIBILITY_VECTORS_DIR
                )
                print(f"  ✓ Synced {vector_count} compatibility vector files")
            except (OSError, shutil.Error, RuntimeError) as exc:
                print(f"\n✗ Failed to sync compatibility vectors: {exc}", file=sys.stderr)
                sys.exit(1)

        if sync_request_signing_vectors:
            try:
                vector_count = sync_request_signing_vectors_from_bundle(
                    bundle_root,
                    REQUEST_SIGNING_VECTORS_DIR,
                    REQUEST_SIGNING_MANIFEST,
                    effective_version=effective_version,
                )
                print(f"  ✓ Synced and pinned {vector_count} request-signing vector files")
            except (OSError, shutil.Error, RuntimeError) as exc:
...
def main() -> None:
    ...
    parser.add_argument(
        "--no-skills",
        action="store_true",
        help="Skip skill sync; schemas and selected compliance vectors still sync.",
    )
